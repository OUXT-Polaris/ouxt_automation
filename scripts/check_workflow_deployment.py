...
ignore_list = [
    "description/wamv_description:BuildTest",
    "description/wamv_description:Release",
]
    ...
    with open(repos_filepath) as file:
        ...
        for repository in repos_yaml["repositories"]:
            ...
            if "OUXT-Polaris" in url and f"{repository}:{workflow_name}" in ignore_list:
                print(
                    f"❗ `{workflow_name}` in {repository} is marked ignored, so it's skipped."
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="python script for overwriting repos file"
    )
